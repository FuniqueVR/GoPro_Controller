...
void CameraListWindow::draw_group_state(const CameraInfo& c){
    ...
    {    
        ...
        if(ImGui::IsItemHovered()){
            ...
            displayText += "serial: ";
            if(c.hw.is_object() && c.hw.contains("serial_number") && c.hw["serial_number"].is_string()){
                displayText += c.hw["serial_number"].get<std::string>();
            }
            displayText += "\n";

            displayText += "model: ";
            if(c.hw.is_object() && c.hw.contains("model_name") && c.hw["model_name"].is_string()){
                displayText += c.hw["model_name"].get<std::string>();
            }
            displayText += "\n";

            displayText += "firmware: ";
            if(c.hw.is_object() && c.hw.contains("firmware_version") && c.hw["firmware_version"].is_string()){
